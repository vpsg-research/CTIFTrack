...
from lib.models.ctiftrack.TIF import featureStorage

from lib.models.ctiftrack.TIF import TIF
...
class SelfAttention(torch.nn.Module):
    def __init__(self, emb_dim, num_heads):
        ...
        self.W_q = torch.nn.Conv1d(64, 64,kernel_size= 1 ,  bias=False)
        self.W_k = torch.nn.Conv1d(64, 64, kernel_size= 1 , bias=False)
        self.W_v = torch.nn.Conv1d(64, 64, kernel_size= 1 , bias=False)
...
class VIT_Backbone(nn.Module):
    def __init__(self,
        ...
        self.liner = nn.Linear(768,768)
        # M1
        self.convz3 = nn.Conv1d(64, 64, 3, 1, 1)
        self.conv13 = nn.Conv1d(64, 64, 3, 1, 1)
        self.conv15 = nn.Conv1d(64, 64, 5, 1, 2)
        self.conv17 = nn.Conv1d(64, 64, 7, 1, 3)
        self.conv19 = nn.Conv1d(64, 64, 9, 1, 4)

        self.convtz1 = nn.Conv1d(256, 64, 3, 1, 1)
        self.convtz2 = nn.Conv1d(256, 64, 3, 1, 1)
        #-------------------------------------------
    ...
    def forward_zx(self, z, x, target_token=None):
        ...
        for i in range(self.num_FE_layers):
            x = self.blocks[i](x, 0)
        featureStorage.store_feature(x)
        previous_features = self.tif.forward()
        x = self.matching(previous_features, x )
        ...
        M = self.convtz1(torch.cat((q1, q2, q3, q4),dim=1))
        SA = self.convtz2(torch.cat((M1, M2, M3, M4),dim=1))
...
def CAE_Base_patch16_224_Async(pretrained=False, **kwargs) -> VIT_Backbone:
    tif = TIF(768, 1024, 768 * 2)
    selfAtt = SelfAttention(768, 8)
