...
class NFSDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_info):
        ...
        canKao_dir = sequence_info['anno_path'].split('/')[1]
        canKao_dir = canKao_dir.split('.')[0]
        init_omit = 0
        if 'initOmit' in sequence_info:
            init_omit = sequence_info['initOmit']
        # print(sequence_path[4:])
        frames = ['{base_path}/{canKao_dir}/{sequence_path}/{frame:0{nz}}.{ext}'.format(base_path=self.base_path, 
        canKao_dir=canKao_dir,sequence_path=sequence_path, frame=frame_num, nz=nz, ext=ext) for frame_num in range(start_frame+init_omit, end_frame+1)]
        
        anno_path = '{}/{}/{}'.format(self.base_path,canKao_dir,sequence_info['anno_path'])
        # print(anno_path)
